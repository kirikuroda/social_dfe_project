...
> **Note on compute time:** The Bayesian parameter estimation in `analysis/02_modeling.R` is computationally intensive and can take several hours to complete. The estimation scripts assume **80 CPU cores** (4 chains × 20 threads per chain). Fitted model objects are cached as `.rds` files in [output/fit/](output/fit/).
