// SMOOTHING VARIANT of group_constant.stan: the Laplace constant added to the
// social choice counts before taking the log-odds is 1.0 here, not 0.1.
// Identical in every other respect (same parameter rows, same data block),
// so the two models are directly comparable under LOO.
//
//   social_lo = log((social_B + 1.0) / (social_A + 1.0))
//
// The constant governs how strongly a lopsided count is shrunk toward
// indifference: with c = 0.1 a single predecessor already yields
// log(1.1/0.1) = 2.40, whereas c = 1.0 yields log(2/1) = 0.69. It enters all
// three places social information is used -- the sampling cue (cue_social),
// the stopping stage (s_t, align_t), and the consequential choice (social_lo).
//
// NOTE: social_scale (the unit-SD scales for |s_t| and align) MUST be
// recomputed with the SAME constant before fitting; 02_modeling.R builds
// stan_data_group_smooth1 for exactly this purpose.
//
// Group condition mixture model with a constant pi_copy per subject.
//
// The consequential choice is a mixture of an asocial and a social
// logistic channel; pi_copy is a per-subject constant (not trial-level).
//
//   asocial_t  = tau_mean * delta_t,  delta_t = (mean_B - mean_A) / se_diff
//   social_lo  = log((n_B + 1.0) / (n_A + 1.0))
//   p(B)       = (1 - pi_copy) * sigma(asocial_t)
//              +      pi_copy  * sigma(theta * social_lo)
//   tau_mean is the inverse temperature on the standardized scale.
//
// Sampling-stage drivers (all in {-0.5, 0, +0.5}, +0.5 points to B,
// except cue_social which is smoothed and lives in (-0.5, +0.5)), plus a
// baseline sampling intercept (gamma_intercept):
//   cue_value     sign of estimated mean difference
//   cue_estim     sign of estimation-variance difference
//   cue_discover  prefers options with unobserved unique outcomes
//   cue_switch    switching away from the just-sampled option
//   cue_social    tanh(log((n_B + 1.0)/(n_A + 1.0))) / 2  (cumulative)
//
...
//
// Parameter rows (mu has 19 entries):
...
//  18  pi_copy                mixture weight (logit-space; transformed via inv_logit)
//  19  theta                  social log-odds weight
    ...
    for (trial in start:end) {
      ...
      for (t in 1:T) {
        ...
        if (t == 1) {
          ...
          real social_lo_pre = log((social_B + 1.0) / (social_A + 1.0));
        ...
        // --------------------------------------------------------
        {
          ...
          if (social_A > 0 || social_B > 0) {
            real social_lo_t = log((social_B + 1.0) / (social_A + 1.0));
          ...
          if (t == T) {
            // Consequential choice. Asocial value signal = single standardized
            // value signal:
            ...
            if (social_A == 0 && social_B == 0) {
              ...
              real social = theta[subj] * log((social_B + 1.0) / (social_A + 1.0));
              lp += bernoulli_lupmf(chose_B | (1 - pi_copy[subj]) * inv_logit(asocial)
                                             + pi_copy[subj] * inv_logit(social));
            }
          }
        }
      }
...
parameters {
  // Hyperparameters [19 rows total]; see header for row map.
  vector[19]          mu;
  vector<lower=0>[19] sigma_group;  // group-level standard deviations
  vector<lower=0>[19] sigma;        // individual-level standard deviations

  // Group-level non-centered deviations (always NCP)
  matrix[19, N_group] z_group;

  // Centered subject-level rows (same rows centered as in solo.stan):
  //   row 1  = beta_intercept       (stopping intercept)
  //   row 2  = beta_step       ((t - t_offset) slope)
  //   row 15 = gamma_switch      (cue_switch switching)
  vector[N_subj] beta_intercept;
  vector[N_subj] beta_step;
  vector[N_subj] gamma_switch;

  // Non-centered z for the remaining 16 rows
  matrix[16, N_subj] z;
}

transformed parameters {
  // Group-level deviations from population mean (non-centered)
  matrix[19, N_group] group_effects = diag_pre_multiply(sigma_group, z_group);

  // Per-subject effective mean (population mean + their group's deviation)
  matrix[19, N_subj] mu_subj = rep_matrix(mu, N_subj)
  ...
  vector<lower=0, upper=1>[N_subj] pi_copy
    = inv_logit(mu_subj[18]' + sigma[18] * z[15]');            // mixing weight
  vector[N_subj] theta                 = mu_subj[19]' + sigma[19] * z[16]';  // social log-odds weight
}
