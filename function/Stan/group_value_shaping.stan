// Group condition value-shaping model.
//
// Replaces the mixture-based social use of group_constant.stan with a perception
// distortion: social consensus on a sampled option amplifies its
// perceived payoff via a per-subject lambda. The consequential choice uses a
// single standardized value signal on the value-shaped beliefs:
//   logit = tau_mean * delta_t,  delta_t = (mean_B - mean_A) / se_diff
// with tau_mean the inverse temperature on the standardized scale. No separate
// social channel at choice time.
//
//   perceived outcome_t = outcome * (1 + lambda * social_share)
//   mean_A, mean_B       running means of perceived outcomes
//   delta_t              z-score of (mean_B - mean_A)
//
// Sampling-side means/variances are computed from the MLE empirical
// frequency p_high = n_high / n (non-shaped), matching the other
// group models; only the stopping/choice sub-model uses the
// value-shaped running means.
//
...
// s_t = tanh(social_lo_t), align_t = u_t * s_t, on the value-shaped delta_t.
// k is data-supplied. (outcome_z is the raw, non-shaped standardized outcome.)
//
...
//
// Parameter rows (mu has 18 entries):
...
//  17  tau_mean               choice slope on delta_t (value-shaped standardized mean diff; inverse temperature)
//  18  lambda                 value-shaping coefficient
...
functions {
  ...
  real partial_sum_lpmf(
    ...
    for (trial in start:end) {
      ...
      real val_A_high = 0;  // current best estimate of A's high outcome
      real val_A_low  = 0;  // current best estimate of A's low outcome
      real val_B_high = 0;  // current best estimate of B's high outcome
      real val_B_low  = 0;  // current best estimate of B's low outcome

      real social_A = 0;  // cumulative social choice count for A
      real social_B = 0;  // cumulative social choice count for B

      // Value-shaped sample means (social consensus amplifies perceived value via lambda).
      real mean_A = 0;
      real mean_B = 0;
      real n_A    = 0;
      real n_B    = 0;
      ...
      for (t in 1:T) {
        ...
        // Sampling-side means/variances are computed from the MLE
        // empirical frequency p_high = n_high / n (non-shaped),
        // matching group_constant.stan. Only the stopping/choice sub-model
        // below uses the value-shaped running means (mean_A, mean_B).
        // --------------------------------------------------------
        if (t == 1) {
        ...
        // --------------------------------------------------------
        // UPDATE BELIEFS with observed outcome at step t (value-shaped)
        // --------------------------------------------------------
        ...
        if (option_sampled[seg_idx] == 0) {
          ...
          n_A += 1;
          // Value shaping: social consensus scales the perceived outcome
          mean_A += (outcome * (1 + lambda[subj] * a_chosen[seg_idx]) - mean_A) / n_A;
        } else {
          // Option B sampled
          if (is_higher_outcome[seg_idx] == 1) {
            obs_B_high = outcome;
          ...
          n_B += 1;
          // Value shaping: social consensus scales the perceived outcome
          mean_B += (outcome * (1 + lambda[subj] * b_chosen[seg_idx]) - mean_B) / n_B;
        }
        ...
        // --------------------------------------------------------
        {
          ...
          // Variance of each option's outcome distribution (around value-shaped mean)
          real var_A;
          real var_B;
          if (obs_A_high < 0 || obs_A_low < 0) {
            var_A = 0.01;  // both outcomes not yet observed
          } else {
            var_A = p_high_A * square(mean_A - val_A_high)
                  + (1 - p_high_A) * square(mean_A - val_A_low);
          }
          if (obs_B_high < 0 || obs_B_low < 0) {
            var_B = 0.01;  // both outcomes not yet observed
          } else {
            var_B = p_high_B * square(mean_B - val_B_high)
                  + (1 - p_high_B) * square(mean_B - val_B_low);
          }

          // SE of the difference in value-shaped sample means
          real se_diff;
          ...
          // Own evidence strength: |u_t| = |tanh(delta_t / k)| (value-shaped delta_t).
          ...
          // once; n_A / n_B are the running sample counts AFTER the step-t
          // update, so this implies t >= 2 and subsumes the old `t > 1` guard.
          // Ineligible steps are not decision points and contribute no stopping
          // term. The terminal step t == T is always scored as a stop, covering
          // trials that end at the task's 40-sample cap.
          ...
          if (t == T) {
            // Consequential choice: single standardized value signal on the
            // value-shaped beliefs (mean_A/B and var_A/B carry the lambda
            // distortion via delta_t):
            //   logit = tau_mean * delta_t,  delta_t = (mean_B - mean_A)/se_diff.
...
data {
  ...
  // pooled with solo.stan) plus group-only social_scale. (|u_t| / align here
  // are value-shaped; the scales are the common non-shaped references, shared
  // across the four group models.)
...
parameters {
  // Hyperparameters [18 rows total]; see header for row map.
  vector[18]          mu;
  vector<lower=0>[18] sigma_group;  // group-level standard deviations
  vector<lower=0>[18] sigma;        // individual-level standard deviations

  // Group-level non-centered deviations (always NCP)
  matrix[18, N_group] z_group;
  ...
  // Non-centered z for the remaining 15 rows
  matrix[15, N_subj] z;
}

transformed parameters {
  // Group-level deviations from population mean (non-centered)
  matrix[18, N_group] group_effects = diag_pre_multiply(sigma_group, z_group);

  // Per-subject effective mean (population mean + their group's deviation)
  matrix[18, N_subj] mu_subj = rep_matrix(mu, N_subj)
  ...
  vector[N_subj] tau_mean              = mu_subj[17]' + sigma[17] * z[14]';  // choice: delta_t (value-shaped standardized mean diff; inverse temperature)
  vector[N_subj] lambda                = mu_subj[18]' + sigma[18] * z[15]';  // value-shaping coefficient
}
